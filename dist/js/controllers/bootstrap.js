...
$scope.setPagingData(data,page,pageSize);
    ...
    $scope.okTratamiento = function (item) {
      ...
      item.idAnimal=idAnimal;
      item.idUsuario=MyService.data.idUsuario;
      $http.post('http://localhost:1337/tratamiento/' ,item);  
      $modalInstance.close();
      // var pas = item._id;
      // $scope.ordenosFiltrados = $scope.ordenos.filter(function (ordeno) {
      //   return (ordeno.idAnimal == pas );
    ...
    $scope.okOrdeno = function (item) {
      ...
      item.idAnimal=idAnimal;
      item.idUsuario=MyService.data.idUsuario;
      $http.post('http://localhost:1337/ordeno/' ,item);  
      $modalInstance.close();
      // var pas = item._id;
      // $scope.ordenosFiltrados = $scope.ordenos.filter(function (ordeno) {
      //   return (ordeno.idAnimal == pas );
  ...
  $scope.cargaMachos();
    ...
    $scope.cargaBanos = function(){
      $http.get('http://localhost:1337/bano/?idUsuario='+MyService.data.idUsuario).then(function (resp) {
...
$scope.okMedicamento = function (item) {
    ...
    // };
      ...
      // item.idAnimal=idAnimal;
       item.idUsuario=MyService.data.idUsuario;
      $http.post('http://localhost:1337/medicamento/' ,item);       
      $modalInstance.close();
    //   var pas = item._id;
 ...
 $scope.okSumBano = function (item) {
    ...
    // };
      var idAnimal=MyService.data.identificador;
       item.idAnimal=idAnimal;
       item.idUsuario=MyService.data.idUsuario;
      $http.post('http://localhost:1337/sumBano/' ,item);       
      $modalInstance.close();
    //   var pas = item._id;
    //   $scope.banosFiltrados = $scope.banos.filter(function (bano) {  
    //   return (bano.idAnimal == pas );
...
// ##########################################################################
    $scope.okConfirm = function (item) { 
      var idAnimal=MyService.data.identificador;
      $http.delete('http://localhost:1337/animal/'+idAnimal , item)
...
//   };
  ...
  ; 
  app.controller('TypeaheadDemoCtrl', ['$scope', '$http','MyService', function($scope, $http, MyService) {
