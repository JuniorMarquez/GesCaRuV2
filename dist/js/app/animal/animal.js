...
app.controller('AnimalCtrl', ['$scope', '$state','$http', '$filter', '$modal', 'MyService', 'filterFilter', 'toaster',  function($scope,  $state ,$http, $filter,$modal, MyService, filterFilter, toaster) {
 ...
 $scope.cargaAlimentos = function(){
  ...
  $scope.deleteItem = function(item){
    $http.delete('http://localhost:1337/animal/'+item.id , item)
  ...
  $scope.deleteAnimal = function(animal){
    $http.delete('http://localhost:1337/animal/'+animal.id , animal)
  ...
  $scope.doneEditingGrupo = function(item){
    ...
    if (MyService.data.idenGen){
      $http.put('http://localhost:1337/grupo/'+MyService.data.idenGen, grupoAct)
    }
    else {
      $http.post('http://localhost:1337/grupo/', grupoAct)
    }
    $http.get('http://localhost:1337/grupo/?idUsuario='+MyService.data.idUsuario).then(function (resp) {
      $scope.groups = resp.data.results;
    });
  ...
  animalAct.estado=item.estado;
   animalAct.control=false;
    ...
    if (MyService.data.idenAnimal){
      $scope.pop4();
      $http.put('http://localhost:1337/animal/'+MyService.data.idenAnimal , animalAct)
    }
    else {
      $scope.pop3();;
      $http.post('http://localhost:1337/animal/', animalAct)
    }
    $http.get('http://localhost:1337/grupo/?idUsuario='+MyService.data.idUsuario).then(function (resp) {
      $scope.groups = resp.data.results;
    });
