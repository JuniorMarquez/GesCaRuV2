'use strict';
app.controller('historicoAnimalCtrl', ['$scope', '$http', '$filter', '$modal', 'MyService', 'filterFilter', function($scope, $http, $filter,$modal, MyService,filterFilter) {
  $scope.filter = '';
      $scope.tbOptions = {
      bDestroy: true,
      data: [],
...
$scope.getOrdenos = function () {
  ...
  $http.get('http://localhost:1340/ordeno/?idAnimal='+MyService.data.animalConsultado.id).then(function (resp) {
    ...
    // alert("cantidad: "+cant);
    $scope.ordenos.nombre=MyService.data.animalConsultado.nombre;
